...
A [hotspot=basicRegistry file=0]`basicRegistry` element is configured in the [hotspot file=0]`server.xml` file in `system` microservice. Only the registered user is authorized to access the `system` microservice. Note that this `basicRegistry` element is a very simple case for learning purposes. For more information about the different user registries, see the https://openliberty.io/docs/latest/user-registries-application-security.html[User registries documentation^].
