...
A [hotspot=basicRegistry file=0]`basicRegistry` element is configured in the `system` server configuration file that is located at `system/src/main/liberty/config/server.xml`. Only the registered user is authorized to access the `system` microservice. Note that this `basicRegistry` element is a very simple case for learning purposes. For more information about the different user registries, see the https://openliberty.io/docs/latest/user-registries-application-security.html[User registries documentation^].
