...
:page-seo-title: Configuring microservices: advanced features and functionality with MicroProfile Config API
...
= Configuring microservices: advanced features and functionality with MicroProfile Config API
...
Inject the `system.properties` property by using the [hotspot=systemPropertiesInject file=1]`@Inject` and [hotspot=systemPropertiesProperty file=1]`@ConfigProperty` annotations with a variable type of [hotspot=systemProperties file=1]`List<String>`.

The [hotspot=systemProperties file=1]`systemProperties` variable retrieves the list of system properties from the configuration and passes it to the [hotspot=getSystemPropertiesMethod file=1]`getSystemProperties()` method.
...
Inject the [hotspot=contactEmail file=1]`query.contactEmail` property as a [hotspot=configValue file]`ConfigValue` type, and add the [hotspot=getContactConfig file=1]`getContactConfig()` class method.
...
Then, inject the [hotspot=config file=1]`Config` metadata object, and add the [hotspot=getConfigSources file=1]`getConfigSources()` and the [hotspot=getConfigProperties file=1]`getConfigProperties()` class methods.
...
MicroProfile Config provides a practical way to aggregate related configuration properties into a single CDI bean by using the `@ConfigProperties` annotation. This annotation is applied to a plain old Java object (POJO) that serves as a holder for configuration properties, making it easier to retrieve. By injecting the annotated POJO as a CDI bean into a separate POJO, a cleaner and more organized approach to managing configuration properties is achieved.
